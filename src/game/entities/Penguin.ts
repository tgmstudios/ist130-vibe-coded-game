...
export class Penguin extends PhysicsBody {
  ...
  public draw(ctx: CanvasRenderingContext2D): void {
    ...
    ctx.fillStyle = '#1a1a2e';
    ctx.beginPath();
    ctx.arc(0, -height / 2 - 4, 10, 0, Math.PI * 2);
    ctx.fill();

    // White face
    ctx.fillStyle = '#ffffff';
    ctx.beginPath();
    ctx.ellipse(0, -height / 2 - 2, 8, 6, 0, 0, Math.PI * 2);
    ctx.fill();
    ...
    ctx.beginPath();
    ctx.arc(-3, -height / 2 - 2, 2, 0, Math.PI * 2);
    ctx.arc(3, -height / 2 - 2, 2, 0, Math.PI * 2);
    ctx.fill();
    ...
    if (Math.abs(this.velocityX) > 10) {
      ...
      ctx.fillStyle = '#ffa500';
      ctx.fillRect(-6, height / 2 - 4 + footOffset, 4, 3);
      ctx.fillRect(2, height / 2 - 4 - footOffset, 4, 3);
    } else {
      ctx.fillStyle = '#ffa500';
      ctx.fillRect(-6, height / 2 - 4, 4, 3);
      ctx.fillRect(2, height / 2 - 4, 4, 3);
    }

    // Scarf (warm accent)
    ctx.fillStyle = '#ff6b6b';
    ctx.fillRect(-8, -height / 2 + 4, 16, 4);
    ctx.fillStyle = '#ff8787';
    ctx.fillRect(-6, -height / 2 + 6, 12, 2);
